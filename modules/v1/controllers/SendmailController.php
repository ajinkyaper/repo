    ...
    public function actionPushmail()
    {
        ...
        if (count($TrailSubmissions) > 0) {
            foreach ($TrailSubmissions as $req) {
                ...
                $query = new Query;
                $query->select(['products.pro_name', 'products.pro_des', 'products.pro_id', 'submission_products.req_id', 'SUBSTR(products.pro_img ,21) as pro_img', 'products.des_url', 'products.cta'])
