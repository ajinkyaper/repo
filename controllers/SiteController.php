...
and (added_on BETWEEN :frm_date and :to_date)")
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $user_model = User::findOne(['username' => $model->username]);
    ...
    public function actionOtpverification()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $user_model = User::findOne(['id' => $otp_model->user_id]);
    ...
    public function ssoAuthorize($data)
    {
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://oauthd.dxc.mktg.run/verify',
    ...
    public function actionSsoLogin()
    {
        ...
        if (!empty(Yii::$app->request->get('token'))) {
            ...
            $user_model = User::findOne(['email' => $response->email]);
            if (empty($user_model)) {
                $user_model =  new User();
                $password = 'Dreambridge123';
                $user_model->email = $response->email;
                $user_model->username = $response->email;
                $user_model->first_name = $response->firstName;
                $user_model->last_name = $response->lastName;
                $user_model->setPassword($password);
                $user_model->save(false);
            } else if (!empty($response->email) && !empty($user_model)) {
                $user_model->first_name = $response->firstName;
                $user_model->last_name = $response->lastName;
                $user_model->save(false);
            }
    ...
    {
        return $this->render('sso_unauthorize', ['name' => 'You are not authorize to login !']);
    }
}
